...
class MuZeroConfig:
    def __init__(self):
        ...
        self.stacked_observations = 32  # Number of previous observations and previous actions to add to the current observation 这个参数是为了让MuZero能够记住之前的状态，保留前32帧的图像信息
        ...
        self.max_moves = 27000  # Maximum number of moves if game is not finished before 游戏的最大步数
        ...
        self.training_steps = int(1000e3)  # Total number of training steps (ie weights update according to a batch) 总共的训练步数
        ...
        self.num_unroll_steps = 5  # Number of game moves to keep for every batch element 未来展开的步数，用于学习状态转移
        self.td_steps = 10  # Number of steps in the future to take into account for calculating the target value 决定使用多少步后的奖励来计算目标值 类似于 TD(n) 算法中的 n 平衡即时奖励和长期收益
